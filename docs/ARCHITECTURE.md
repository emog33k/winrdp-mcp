...
graph TD
    ...
    subgraph server["FastMCP server — server.py"]
        Tools["144 @mcp.tool functions<br/>tools/*.py"]
...
| `winrdp_mcp/tools/` | The 144 tools across `hosts`, `provisioning`, `system`, `scripting`, `files`, `admin`, `rdp`, `software`, `network`, `windows`, `gui`, `waiters`, `scheduling`, `tunnel`, `ops` — each exposes `register(mcp, ctx)`. `PROFILES` + `register_all(..., profile=)` gate which modules load. |
| `winrdp_mcp/prompts.py` / `resources.py` | MCP prompts (guided workflows) and resources (inventory + live host summary) |
