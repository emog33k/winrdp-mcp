...
graph TD
    ...
    subgraph server["FastMCP server — server.py"]
        Tools["108 @mcp.tool functions<br/>tools/*.py"]
...
| `winrdp_mcp/tools/` | The 108 tools across `hosts`, `provisioning`, `system`, `scripting`, `files`, `admin`, `rdp`, `software`, `network`, `windows` — each exposes `register(mcp, ctx)` |
